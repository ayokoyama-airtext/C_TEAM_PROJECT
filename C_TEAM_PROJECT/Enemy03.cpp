...
const float CEnemy03::ROTATION_SPEED = 0.025f;
const float CEnemy03::ENEMY_SPEED = 5.f;
const float CEnemy03::ENEMY_ESCAPE_SPEED = 8.f;
const float CEnemy03::ENEMY_ESCAPE_ROTATION_SPEED = 0.2f;
const float CEnemy03::ENEMY_ESCAPE_ANGLE = cosf(PI * 0.5f);
const float CEnemy03::SEARCH_ANGLE = cosf(PI * 0.5f);

float CEnemy03::m_fFieldWidth = 0.f;
float CEnemy03::m_fFieldHeight = 0.f;
ID2D1Bitmap *CEnemy03::m_pImage = NULL;
CStage *CEnemy03::m_pParent = NULL;
#ifdef _DEBUG
ID2D1SolidColorBrush *CEnemy03::m_pBrush = NULL;
ID2D1SolidColorBrush *CEnemy03::m_pRedBrush = NULL;
#endif // _DEBUG



CEnemy03::CEnemy03(float x, float y, float scale)
{
	m_fX = x;
	m_fY = y;
	m_fVX = 0;
...
*@brief	描画メソッド
*/
void CEnemy03::draw(ID2D1RenderTarget *pRenderTarget) {
	float playerX = m_pParent->playerCoords.playerX;
	float playerY = m_pParent->playerCoords.playerY;
...
*/
bool CEnemy03::collide(float x, float y, float r) {
	float vx = m_fX - x;
	float vy = m_fY - y;
...
*@return	true 当たり / false 外れ
*/
bool CEnemy03::collide(IGameObject *pObj) {
	if (m_iDamagedTimer > 0)
		return false;
...
void CEnemy03::damage(float amount) {
	if (m_iDotNum > 0) {
...
*@brief	Radを返す
*/
float CEnemy03::GetRad() {
	return m_fRad;
}
/**
*@brief	Angleを返す
*/
float CEnemy03::GetAngle() {
	return m_fAngle;
}
...
*/
void CEnemy03::Restore(ID2D1RenderTarget *pTarget, CStage *pStage) {
