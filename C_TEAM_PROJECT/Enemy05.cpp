...
const float CEnemy05::ROTATION_SPEED = 0.025f;
const float CEnemy05::ENEMY_SPEED = 5.f;
const float CEnemy05::ENEMY_ESCAPE_SPEED = 8.f;
const float CEnemy05::ENEMY_ESCAPE_ROTATION_SPEED = 0.2f;
const float CEnemy05::ENEMY_ESCAPE_ANGLE = cosf(PI * 0.5f);
const float CEnemy05::SEARCH_ANGLE = cosf(PI * 0.5f);

float CEnemy05::m_fFieldWidth = 0.f;
float CEnemy05::m_fFieldHeight = 0.f;
ID2D1Bitmap *CEnemy05::m_pImage = NULL;
CStage *CEnemy05::m_pParent = NULL;
#ifdef _DEBUG
ID2D1SolidColorBrush *CEnemy05::m_pBrush = NULL;
ID2D1SolidColorBrush *CEnemy05::m_pRedBrush = NULL;
#endif // _DEBUG



CEnemy05::CEnemy05(float x, float y, float scale)
{
	m_fX = x;
	m_fY = y;
	m_fVX = 0;
...
*@brief	描画メソッド
*/
void CEnemy05::draw(ID2D1RenderTarget *pRenderTarget) {
	float playerX = m_pParent->playerCoords.playerX;
	float playerY = m_pParent->playerCoords.playerY;
...
*/
bool CEnemy05::collide(float x, float y, float r) {
	float vx = m_fX - x;
	float vy = m_fY - y;
...
*@return	true 当たり / false 外れ
*/
bool CEnemy05::collide(IGameObject *pObj) {
	if (m_iDamagedTimer > 0)
		return false;
...
void CEnemy05::damage(float amount) {
	if (m_iDotNum > 0) {
...
*@brief	Radを返す
*/
float CEnemy05::GetRad() {
	return m_fRad;
}
/**
*@brief	Angleを返す
*/
float CEnemy05::GetAngle() {
	return m_fAngle;
}
...
*/
void CEnemy05::Restore(ID2D1RenderTarget *pTarget, CStage *pStage) {
