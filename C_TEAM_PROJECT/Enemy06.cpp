...
const float CEnemy06::ROTATION_SPEED = 0.025f;
const float CEnemy06::ENEMY_SPEED = 5.f;
const float CEnemy06::ENEMY_ESCAPE_SPEED = 8.f;
const float CEnemy06::ENEMY_ESCAPE_ROTATION_SPEED = 0.2f;
const float CEnemy06::ENEMY_ESCAPE_ANGLE = cosf(PI * 0.5f);
const float CEnemy06::SEARCH_ANGLE = cosf(PI * 0.5f);

float CEnemy06::m_fFieldWidth = 0.f;
float CEnemy06::m_fFieldHeight = 0.f;
ID2D1Bitmap *CEnemy06::m_pImage = NULL;
CStage *CEnemy06::m_pParent = NULL;
#ifdef _DEBUG
ID2D1SolidColorBrush *CEnemy06::m_pBrush = NULL;
ID2D1SolidColorBrush *CEnemy06::m_pRedBrush = NULL;
#endif // _DEBUG



CEnemy06::CEnemy06(float x, float y, float scale)
{
	m_fX = x;
	m_fY = y;
	m_fVX = 0;
...
*@brief	描画メソッド
*/
void CEnemy06::draw(ID2D1RenderTarget *pRenderTarget) {
	float playerX = m_pParent->playerCoords.playerX;
	float playerY = m_pParent->playerCoords.playerY;
...
*/
bool CEnemy06::collide(float x, float y, float r) {
	float vx = m_fX - x;
	float vy = m_fY - y;
...
*@return	true 当たり / false 外れ
*/
bool CEnemy06::collide(IGameObject *pObj) {
	if (m_iDamagedTimer > 0)
		return false;
...
void CEnemy06::damage(float amount) {
	if (m_iDotNum > 0) {
...
*@brief	Radを返す
*/
float CEnemy06::GetRad() {
	return m_fRad;
}
/**
*@brief	Angleを返す
*/
float CEnemy06::GetAngle() {
	return m_fAngle;
}
...
*/
void CEnemy06::Restore(ID2D1RenderTarget *pTarget, CStage *pStage) {
